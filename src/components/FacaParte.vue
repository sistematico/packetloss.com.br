<template>
  ...
  >
    ...
    <h2 class="subtitle">Seja um PKL</h2>
    ...
    <div class="field">
      <label class="label has-text-light">Apelido</label>
      <div class="control">
        <input
          class="input"
          id="apelido"
          v-model="campos.apelido"
          type="text"
          name="apelido"
          placeholder="Ex.: alex"
        />
    ...
    <div class="field">
      ...
      <div class="control">
        <input
          ...
          type="text"
          name="email"
          placeholder="Ex.: alex@google.com"
        />
      </div>
    </div>
  ...
  <article class="message is-success" v-else>
    ...
    <div class="message-body">E-mail enviado com sucesso!</div>
  </article>
</template>
<script>
  ...
  methods: {
    checkForm: function (e) {
      ...
      if (
        ...
        this.campos.email &&
        this.campos.assunto &&
        this.campos.mensagem
      ) {
        axios
          .post("/php/mail.php", this.campos)
          .then((response) => {
      ...
      if (!this.campos.nome) this.errors.push("O nome é obrigatório.");

      if (!this.campos.apelido) this.errors.push("O apelido é obrigatório.");

      if (!this.campos.email) this.errors.push("O email é obrigatório.");

      if (!this.campos.assunto) this.errors.push("O assunto é obrigatório.");

      if (!this.campos.mensagem) this.errors.push("A mensagem é obrigatória.");
    },
  },
};
</script>